    ...
    template: `
        @if (multiple()) {
            <app-multi-select [options]="getOptions"
                ...
                (valueChange)="onMultipleValueChange($event)">
                <ng-content/>
            </app-multi-select>
        } @else {
...
export class RowSelect<T extends TableName> extends InputBase<number | number[]> {
    ...
    readonly onOptionClick = input<(option: SelectOption<number | string>, event: MouseEvent) => void>();
    ...
    protected onMultipleValueChange(value: Array<number | string> | string | null) {
        if (!Array.isArray(value)) {
            this.setViewValue([]);
            return;
        }
        this.setViewValue(value.filter((item): item is number => typeof item === 'number'));
    }
